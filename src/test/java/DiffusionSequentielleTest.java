...
class DiffusionSequentielleTest {
    ...
    void setUp(){
        ...
        strat = new DiffusionSequentielle();
     ...
     * toutes les valeurs de ticks envoyées ou alors aucune
     * pour vérifier que c'est bien une séquence qui est envoyées.
     */
    @Test
    void test() throws InterruptedException {
        ...
        capteur.tick();
        sleep(3000);
        Assertions.assertTrue(allContainsOrNone(1));

        capteur.tick();
        sleep(3000);
        Assertions.assertTrue(allContainsOrNone(2));

        capteur.tick();
        sleep(3000);
        Assertions.assertTrue(allContainsOrNone(3));

        capteur.tick();
        sleep(3000);
        Assertions.assertTrue(allContainsOrNone(4));
     ...
     * @return boolean true, si tous les afficheurs contiennent value ou alors aucun
     */
    private boolean allContainsOrNone(int value){
        return (afficheur.getMemory().contains(value) &&
                afficheur2.getMemory().contains(value) &&
                afficheur3.getMemory().contains(value)) || (
                        !afficheur.getMemory().contains(value) &&
                        !afficheur2.getMemory().contains(value) &&
                        !afficheur3.getMemory().contains(value));
